...
export function EditarEnquete() {
  ...
  const carregarEnquete = async () => {
    try {
      setLoading(true);
      const enquete = await enqueteService.obterPorId(id!);
      setPergunta(enquete.pergunta);
      setOpcoes(enquete.opcoes);
      
      // Separar data e hora do datetime
      const inicio = new Date(enquete.prazoInicio);
      const fim = new Date(enquete.prazoFim);
      
      setDataInicio(formatDate(inicio));
      setHoraInicio(formatTime(inicio));
      setDataFim(formatDate(fim));
      setHoraFim(formatTime(fim));
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await enqueteService.editar(id!, {
      ...
      <div className="mx-auto max-w-2xl px-6 py-12">
        ...
        >
          ...
          <p className="mb-8 text-neutral-400">
            Atualize as informações da sua enquete
          </p>
